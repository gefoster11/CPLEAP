...
#ID <- "NVT06"
#exp_condition <- "IH"
#pre_time <- c(154.7499, 893.4968307, 1126.523196, 1346.96465, 1561.606437) #PRE - provide the desired start times of your intervals
#post_time <- c(4281.206337, 4322.579026, 4544.593981, 4753.662013, 4969.894556) #POST - provide the desired start times of your intervals
...
rbind(pre_burst_stage, cbind(summarise_at(filter(pre_burst, Time >= pre_time[i] & Time < pre_time[i] + 60), c("MSNA_Baseline", "MSNA_Amplitude", "MSNA_Normalized_Amplitude"), mean, na.rm = TRUE), length(pre_burst$Time[pre_burst$Time >= pre_time[i] & pre_burst$Time < pre_time[i] + 60]))) -> pre_burst_stage
}

names(pre_burst_stage)[4] <- "nbursts"
...
for(i in 1:length(stage)) {
  rbind(pre_breath_stage, cbind(summarise_at(filter(pre_breath, Time >= pre_time[i] & Time < pre_time[i] + 60), c("VI", "VT", "Fb", "PO2", "PCO2", "Box.Pressure"), mean, na.rm = TRUE), length(pre_breath$Time[pre_breath$Time >= pre_time[i] & pre_breath$Time < pre_time[i] + 60]))) -> pre_breath_stage
}

names(pre_breath_stage)[7] <- "nbreath"

#PRE - Generate Requested Data Averages for beat file
pre_beat_stage <- NULL

for(i in 1:length(stage)) {
  rbind(pre_beat_stage, cbind(summarise_at(filter(pre_beat, Time >= pre_time[i] & Time < pre_time[i] + 60), c("SPO2", "SBP", "DBP", "MAP", "HR", "SV", "CO", "TPR", "V_ba", "Q_BA", "FVR", "FVC"), mean, na.rm = TRUE), length(pre_beat$Time[pre_beat$Time >= pre_time[i] & pre_beat$Time < pre_time[i] + 60]))) -> pre_beat_stage
}

names(pre_beat_stage)[13] <- "nbeat"

pre_beat_stage[1,9:12] <- c(NaN, NaN, NaN, NaN)
...
for(i in 1:length(stage)) {
  rbind(post_burst_stage, cbind(summarise_at(filter(post_burst, Time >= post_time[i] & Time < post_time[i] + 60), c("MSNA_Baseline", "MSNA_Amplitude", "MSNA_Normalized_Amplitude"), mean, na.rm = TRUE), length(post_burst$Time[post_burst$Time >= post_time[i] & post_burst$Time < post_time[i] + 60]))) -> post_burst_stage
}

names(post_burst_stage)[4] <- "nbursts"
...
for(i in 1:length(stage)) {
  rbind(post_breath_stage, cbind(summarise_at(filter(post_breath, Time >= post_time[i] & Time < post_time[i] + 60), c("VI", "VT", "Fb", "PO2", "PCO2", "Box.Pressure"), mean, na.rm = TRUE), length(post_breath$Time[post_breath$Time >= post_time[i] & post_breath$Time < post_time[i] + 60]))) -> post_breath_stage
}

names(post_breath_stage)[7] <- "nbreath"

#POST - Generate Requested Data Averages for beat file
post_beat_stage <- NULL

for(i in 1:length(stage)) {
  rbind(post_beat_stage, cbind(summarise_at(filter(post_beat, Time >= post_time[i] & Time < post_time[i] + 60), c("SPO2", "SBP", "DBP", "MAP", "HR", "SV", "CO", "TPR", "V_ba", "Q_BA", "FVR", "FVC"), mean, na.rm = TRUE), length(post_beat$Time[post_beat$Time >= post_time[i] & post_beat$Time < post_time[i] + 60]))) -> post_beat_stage
}

names(post_beat_stage)[13] <- "nbeat"

post_beat_stage[1,9:12] <- c(NaN, NaN, NaN, NaN)

post_stage <- cbind(ID, exp_condition, condition, stage, post_breath_stage, post_beat_stage, post_burst_stage)

post_stage <- mutate(post_stage, VT = VT*-1, burst_frequency = nbursts, burst_incidence = (burst_frequency/HR)*100, total_MSNA = MSNA_Amplitude*burst_frequency, total_normalized_MSNA = MSNA_Normalized_Amplitude*burst_frequency)
...
var_list <- colnames(select(all_stage_out, 5:33))
